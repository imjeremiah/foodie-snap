-- Fix Seed Demo Data Query Error
-- This migration fixes the "query has no destination for result data" error

-- Drop and recreate the seed function with fixed query logic
...
BEGIN
  ...
  -- Create conversations (FIXED: removed RETURNING clause)
  INSERT INTO public.conversations (id, created_by, created_at, updated_at) VALUES
  (conversation_1, current_user_id, NOW() - INTERVAL '2 hours', NOW() - INTERVAL '5 minutes'),
  (conversation_2, current_user_id, NOW() - INTERVAL '1 day', NOW() - INTERVAL '2 hours'),
  (conversation_3, demo_friend_2, NOW() - INTERVAL '3 days', NOW() - INTERVAL '1 day');
  ...
  -- Also trigger spotlight content creation if it doesn't exist
  PERFORM public.seed_spotlight_content();
  
  RETURN 'Successfully created demo data! 🎉 You now have 3 demo friends (Alex, Sarah, Mike) with active conversations. Check your chat list and spotlight feed to see them!';
